...
from app.utils import grobid_client
...
def upload_paper(file: UploadFile = File(...)):
    try:
        ...
        # --- Parse with GROBID if available ---
        try:
            tei_xml = grobid_client.parse_pdf_with_grobid(str(paper_path))
            title, authors = pdf_parser.extract_metadata_from_tei(tei_xml)
            sections = pdf_parser.extract_body_sections_from_tei(tei_xml)
        except Exception as e:
            # Fall back to local parser if GROBID fails
            parsed = pdf_parser.parse_pdf(paper_path)
            title, authors = parsed["title"], parsed["authors"]
